...
They are read from the `MOCADEV_FEATURES.txt` at startup, or before each execution if configured via READ_SETTINGS_EVERY_TIME. To enable the MOCA Client to utilize these feature file settings, you need to configure a variable in registry setting.
